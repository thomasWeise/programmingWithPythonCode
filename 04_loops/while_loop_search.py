...
for search in ["a", "c", "o", "p", "w", "z"]:  # Search 6 characters.
    upper: int = len(data) - 1  # Largest possible index = end of str.
    lower: int = 0  # Lowest possible index = 0.
    while lower <= upper:  # Repeat until lower > upper.
        mid: int = (lower + upper) // 2  # mid = index in middle.
        ...
        if mid_str < search:  # If mid_str < search, then clearly...
            ...
            upper = mid - 1  # ...the index of search must be > mid.
